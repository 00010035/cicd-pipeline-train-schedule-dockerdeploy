    ...
    stages {
        ...
        stage ('DeployToProduction') {
            ...
            steps{
                ...
                withCredentials ([usernamePassword(credentialsId: 'webserver_login', usernameVariable: 'deploy', passwordVariable: 'Ghjabkm09@')]) {
                    script {
                        ...
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@${env.prod_ip} \"docker run --restart always --name train-schedule -p 8080:8080 -d fibroxim/train-schedule:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }   
